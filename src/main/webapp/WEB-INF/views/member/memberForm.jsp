		...
		<table class="table table-bordered">
				...
				<td>주소</td>
					<td>
						<p><input type="text" name="mem_zipcode" size="6" value="${member.mem_zipcode}">
							<button type="button" class="btn btn-warning">우편번호검색</button>
						</p>
			...
			<tr>
				<td colspan="2">
					...
					<c:if test="${param.type == 'I' }">
						<input type="button" value="가입하기"  class="btn btn-success" onclick="fn_save('I')">
					</c:if>
					
					<c:if test="${not empty member.mem_id}">
						<input type="button" value="수정하기"  class="btn btn-success" onclick="fn_save('U')">
					</c:if>
				
					<input type="reset" value="취소"  class="btn btn-danger">
					<input type="button" value="돌아가기"  class="btn btn-primary" onclick="fn_list()">
				</td>
			</tr>
		</table>
	</form>	
