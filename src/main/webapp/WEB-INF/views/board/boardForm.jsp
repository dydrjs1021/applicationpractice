...
<title>Insert Card Board</title>
</head>
<body>	
	
<div class="container">
	<h3>${board.bo_seq_no == 0 ? '게시글 작성' : '게시글 수정'}</h3>
	...
	<form name="boardForm" id="boardForm" method="post" enctype="multipart/form-data">
		...
		<table class="table table-bordered">
			...
			<tr>
				...
				<td>
					<div class="col-xs-3">
						<input type="hidden" name="bo_writer" value="${board.bo_writer }" class="form-control">
						<input type="text" name="bo_writer_name" value="${board.bo_writer_name }" class="form-control">
					</div>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<div class="checkbox col-xs-12">
						<label for="bo_open_yn"><input type="checkbox" name="bo_open_yn" id="bo_open_yn" value="Y" ${board.bo_open_yn == 'Y' ? '' : 'checked' }> 공개여부</label>
					</div>
				</td>
			</tr>
			<!-- 첨부파일 -->			
			<%-- <tr>
				...
				<p>
					<button type="button" class="btn btn-primary btn-xs" id="btnAddFile">추가</button>
				</p>
					...
					<div id="fileList">
						...
						<div class="checkbox col-xs-12">
							...
							<!-- multiple == html5에서 기본적으로 지원해 줌 하지만 잘 안쓰임 = 파일을 뭐 넣엇는지 안보여주기때문에-->
						</div>
